...
namespace adore
{
    namespace if_ROS
    {
        ...
        class IndicatorCommandReader:public adore::mad::AReader<adore::fun::IndicatorCommand>
        {
            ...
            {
                bool old_state = data_.getIndicatorLeftOn();
                data_.setIndicatorLeftOn(msg.get()->data);
                changed_ = changed_ || data_.getIndicatorLeftOn() != old_state;
            }
            void receive_right(std_msgs::BoolConstPtr msg)
            {
                bool old_state = data_.getIndicatorRightOn();
                data_.setIndicatorRightOn(msg.get()->data);
                changed_ = changed_ || data_.getIndicatorRightOn() != old_state;
            }
            public:
