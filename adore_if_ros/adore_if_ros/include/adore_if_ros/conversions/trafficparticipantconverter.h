...
namespace adore
{
    namespace if_ROS
    {
        ...
        /**
         * @brief TPSetMultiReader reads two ros topics and combines data from both
         * 
         */
        class TPSetMultiReader:public adore::mad::AReader<adore::env::traffic::TParticipantSet>
        {
            private:
            bool initialized_;
            bool changed_;
                ...
                :initialized_(false),changed_(false)
            {
                ...
                subscriber1_ = n->subscribe(topic1,qsize,&TPSetMultiReader::receive1,this,ros::TransportHints().tcpNoDelay(no_delay));
                subscriber2_ = n->subscribe(topic2,qsize,&TPSetMultiReader::receive2,this,ros::TransportHints().tcpNoDelay(no_delay));
            }
            ...
            {
                value.clear();
                for(const auto& p: data1_)value.push_back(p);
                for(const auto& p: data2_)value.push_back(p);
                changed_ = false;
            }
        };
