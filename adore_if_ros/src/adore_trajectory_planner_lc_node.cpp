...
namespace adore
{
  namespace if_ROS
  {  
    class TrajectoryPlannerLCNode : public Baseapp
    {
      public:
      ...
      void init(int argc, char **argv, double rate, std::string nodename,bool directionLeft,std::string name,int id)
      {
        ...
        Baseapp::initSim();
        planner_ = new adore::apps::TrajectoryPlannerLC(directionLeft,name,id);
...
int main(int argc,char **argv)
{
    ...
    lcn.init(argc, argv, 100.0, "adore_trajectory_planner_lc_node",direction.compare("left")==0,name,id);
    lcn.run();
    return 0;
}