...
namespace adore
{
  namespace if_ROS
  {
    namespace params
    {
      class PFunctionManagement:public adore::params::APFunctionManagement,ROSParam
      {
        public:
        PFunctionManagement(ros::NodeHandle n,std::string prefix)
        :ROSParam(n,prefix + "function_management/")
        {
        }
        virtual double getRadioKillSwitchTimeout()const override
        {
          double value = 5.0;
          static const std::string name = prefix_ + "radio_kill_switch_timeout";
          get(name,value);
          return value;
        }
      };
