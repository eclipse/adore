 ...
 *   Robert Markowski - extension
 *   Thomas Lobig - extensions
 ********************************************************************************/
...
/* object methods */
		Figure::Figure(const char* name,unsigned int id)
			:name(name),id(id),showAxis(0),showGrid(0),mouseOverX(0),mouseOverY(0),
			needsPostHide(false),needsPostShow(false),needsPostRedisplay(false)
		{
			...
			bound_recomputation_necessary = false;
		...
		void Figure::createGlutFigure(const char* name,unsigned int id)
		{
			...
			case 0:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc0);
			...
			case 1:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc1);				
			...
			case 2:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc2);
			...
			case 3:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc3);			
			...
			case 4:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc4);			
			...
			case 5:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc5);			
			...
			case 6:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc6);			
			...
			case 7:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc7);			
			...
			case 8:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc8);		
			...
			case 9:
				...
				glutPassiveMotionFunc(Figure::glut_passiveMotionFunc9);			
		...
		void Figure::display(void)
		{
			...
			pthread_mutex_lock(&(buffer.mut));
				...
				// std::cout << " debug plotobject count " << buffer.plotObjects.size() + buffer.deferredObjects.size() << " - ( " << buffer.deferredObjects.size() << " deferred )" << std::endl;
			...
			{
				GLdouble z;
  				glReadPixels (mouseOverX, h-mouseOverY, 1, 1, GL_DEPTH_COMPONENT, GL_DOUBLE, &z);
				...
				gluUnProject(mouseOverX, h-mouseOverY,0,model_mat,proj_mat,view_mat,&objX,&objY,&objZ);

				GLdouble obj2X;
				GLdouble obj2Y;
				GLdouble obj2Z;
				gluUnProject(mouseOverX, h-mouseOverY,1,model_mat,proj_mat,view_mat,&obj2X,&obj2Y,&obj2Z);

				double factor_a = (z - obj2Z) / (objZ - obj2Z);
				resultant.x_ = (objX - obj2X)*factor_a + obj2X;
				resultant.y_ = (objY - obj2Y)*factor_a + obj2Y;
				resultant.z_ = z;

				glBegin(GL_LINES);
					glColor3f(0.0f,0.3f,0.9f);
					glVertex3d(resultant.x_-5,resultant.y_+5,resultant.z_+0.1);
					glVertex3d(resultant.x_+5,resultant.y_-5,resultant.z_+0.1);
					glVertex3d(resultant.x_+5,resultant.y_+5,resultant.z_+0.1);
					glVertex3d(resultant.x_-5,resultant.y_-5,resultant.z_+0.1);
				glEnd();
				...
				std::stringstream ss;
				ss << std::setprecision(getPrecision(resultant.x_)) << "x: " << resultant.x_ << " - y: " << std::setprecision(getPrecision(resultant.y_)) << resultant.y_ << " - z: " << std::setprecision(getPrecision(resultant.z_)) << resultant.z_;

				const GLdouble color[3] = {0.2,0.9,0.4};
				plotLabelUI(ss.str(),-10,5,color);
			}
		...
		void Figure::plotLabelUI(std::string s, double x,double y, const GLdouble color[3])
		{
			glMatrixMode(GL_PROJECTION);
			glPushMatrix();
			glLoadIdentity();
			...
			glRasterPos3d(x, y,1);
			glutBitmapString(GLUT_BITMAP_8_BY_13,reinterpret_cast<unsigned const char*> (text));
			glMatrixMode(GL_MODELVIEW);
    		glPopMatrix();
    		glMatrixMode(GL_PROJECTION);
    		glPopMatrix();
...
//			glutPostRedisplay();
		...
		void Figure::motionFunction(int x,int y)
		{
			...
			else
			{
				...
				double speedy = ywidth_float / this->w * (1-transZ);
				double speedx = speedy;
				if(axis._auto)
				{
					speedy = ywidth_float / this->h * (1-transZ);
					speedx = xwidth_float / this->w * (1-transZ);
				}
		...
		void Figure::passiveMotionFunction(int x,int y)
		{
			if (logPosition())
			{
				mouseOverX = x;
		...
		void Figure::mouseFunction(int button,int state,int x,int y)
		{
			...
			case GLUT_LEFT_BUTTON:
				...
				{
					long now = clock();
					if (((double)(now-t_mouseDown))/CLOCKS_PER_SEC<0.2)
					{
						if(click_count == 2)
						{
							//double click
						...
						if(click_count == 1)
						{
							if (logPosition())
							{
								Quaternion q(lastPoint,resultant);
								Heading h(lastPoint,resultant);
								logPositionFile << std::fixed << resultant.x_ << "," << resultant.y_ << "," << resultant.z_;
								logPositionFile << std::fixed << "," << h.heading_;
								logPositionFile << std::fixed << "," << q.x_ << "," << q.y_ << "," << q.z_ << "," << q.w_;
								logPositionFile << std::endl;
								lastPoint = resultant;
							}
						}
						if(click_count != 1)
		...
		void Figure::setLogPosition(bool value)
		{
			if(value && !logPosition())
			{
				logPositionFile.open("logged_positions.txt",std::ofstream::out | std::ofstream::app);
				logPositionFile << "begin_record" << std::endl;
				logPositionFile << "point_x, point_y, point_z, heading, quaternion_x, quaternion_y, quaternion_z, quaterion_w" << std::endl;
			}
			else if(!value && logPosition())
			{
				logPositionFile << "end_record" << std::endl;
				logPositionFile.close();
			}
			
		}
		bool Figure::logPosition()
		{
			return logPositionFile.is_open();
		}	
		...
		{
			bound_recomputation_necessary = true;
		}
		void Figure::recomputeBounds_execute()
		{
			if( bound_recomputation_necessary )
			{
				bound_recomputation_necessary = false;
			}
			else
		...
		{
			// char* argv[] = {""}; // this gives a warning, leaving it in as a reminder if we ever need argv here
			int argc = 1;
			glutInit(&argc, 0);
...
/* static methods: */
		...
		void Figure::keyFunction(unsigned char key, int x, int y)
		{
			...
			case 'l'://activate log mode
				...
				break;
			case 'n':// start new line in log mode
				if(logPosition())
				{
					logPositionFile << "line" << std::endl;
				}
				break;
		...
		void Figure::glut_passiveMotionFunc0(int x,int y){static_figure_set.at(0)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc1(int x,int y){static_figure_set.at(1)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc2(int x,int y){static_figure_set.at(2)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc3(int x,int y){static_figure_set.at(3)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc4(int x,int y){static_figure_set.at(4)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc5(int x,int y){static_figure_set.at(5)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc6(int x,int y){static_figure_set.at(6)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc7(int x,int y){static_figure_set.at(7)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc8(int x,int y){static_figure_set.at(8)->passiveMotionFunction(x,y);}
		void Figure::glut_passiveMotionFunc9(int x,int y){static_figure_set.at(9)->passiveMotionFunction(x,y);}
