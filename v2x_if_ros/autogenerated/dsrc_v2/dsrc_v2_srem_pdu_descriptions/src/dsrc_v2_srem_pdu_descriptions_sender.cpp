...
wind::wind_ros::Sender_dsrc_v2_srem_pdu_descriptions::Sender_dsrc_v2_srem_pdu_descriptions(wind::comm::UDPSender* udp_sender) {
...
wind::wind_ros::Sender_dsrc_v2_srem_pdu_descriptions::start()
{
    this->subscriber = node_handle.subscribe(topic, 10, 
        &wind::wind_ros::Sender_dsrc_v2_srem_pdu_descriptions::callback, this);
    ROS_INFO_STREAM("Subscribing to topic: " << topic);

    this->mirrorSubscriber = node_handle.subscribe(SENDER_ROS_TOPIC_MIRROR, 10, 
        &wind::wind_ros::Sender_dsrc_v2_srem_pdu_descriptions::mirrorCallback, this);
}

void
wind::wind_ros::Sender_dsrc_v2_srem_pdu_descriptions::callback(
        const dsrc_v2_srem_pdu_descriptions::SREM::ConstPtr& msg)
{
    wind::cpp::SREM_PDU_Descriptions::SREM wind = {};
    ros2wind(msg, &wind);
    ...
    ROS_INFO_STREAM("[" << ros::this_node::getName() << "] Sending UDP message (" << sizeof(wind::cpp::SREM_PDU_Descriptions::SREM) << ") to " << output_ip << ":" << output_port);
    this->udp_sender->send(&wind, sizeof(wind::cpp::SREM_PDU_Descriptions::SREM));
}

void
wind::wind_ros::Sender_dsrc_v2_srem_pdu_descriptions::mirrorCallback(
        const dsrc_v2_srem_pdu_descriptions::SREM::ConstPtr& msg)
{
    wind::cpp::SREM_PDU_Descriptions::SREM wind = {};
    ros2wind(msg, &wind);

    ROS_INFO_STREAM("***** Outgoing SREM request message:  counter(" << messagesCounter++ << ") *****");
    ...
    ROS_INFO_STREAM("[" << ros::this_node::getName() << "] [Mirror] Sending UDP message (" << sizeof(wind::cpp::SREM_PDU_Descriptions::SREM) << ") to " << output_ip << ":" << output_port);
    this->udp_sender->send(&wind, sizeof(wind::cpp::SREM_PDU_Descriptions::SREM));
}
