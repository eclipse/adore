...
public:
    ...
    ~ReceiverNode() {
        delete udp_receiver;
        delete receiver_dsrc_v2_spatem_pdu_descriptions;
    }
    ...
    void receiveMsgCallback(const void* buf, const size_t& buf_size)
    {
        ...
        if(buf_size == receiver_dsrc_v2_spatem_pdu_descriptions->message_id()) {
            receiver_dsrc_v2_spatem_pdu_descriptions->incoming_message(buf);
        }
        else {
    ...
    void run()
    {
        ...
        receiver_dsrc_v2_spatem_pdu_descriptions = new wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions();
        ROS_INFO_STREAM("Registering message name(" << receiver_dsrc_v2_spatem_pdu_descriptions->message_name() 
            << ") id(" << receiver_dsrc_v2_spatem_pdu_descriptions->message_id() << ")");
        receiver_dsrc_v2_spatem_pdu_descriptions->set_topic(topic);
        receiver_dsrc_v2_spatem_pdu_descriptions->start();
...
private:
    ...
    wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions* receiver_dsrc_v2_spatem_pdu_descriptions;
};
