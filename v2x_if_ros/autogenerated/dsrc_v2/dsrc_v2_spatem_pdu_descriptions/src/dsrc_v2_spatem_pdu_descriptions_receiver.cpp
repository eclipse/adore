 ...
 * Constructor
 */
wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions::Receiver_dsrc_v2_spatem_pdu_descriptions() : node_handle("~")
{
    messagesCounter = 0;
}
...
wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions::start() {
    publisher        = node_handle.advertise<dsrc_v2_spatem_pdu_descriptions::SPATEM>(topic, 0);
    mirrorPublisher  = node_handle.advertise<dsrc_v2_spatem_pdu_descriptions::SPATEM>(RECEIVER_ROS_TOPIC_MIRROR, 0);
}

uint32_t
wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions::message_id() {
    return sizeof(wind::cpp::SPATEM_PDU_Descriptions::SPATEM);
}


void
wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions::set_topic(std::string t) {
    topic = t;
}

std::string
wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions::message_name() {
    return "SPATEM";
}
...
wind::wind_ros::Receiver_dsrc_v2_spatem_pdu_descriptions::incoming_message(const void* buf)
{
    ...
    if(bit3)
        ROS_INFO_STREAM("***** Incoming SPATEM TEST indication message: counter(" << messagesCounter++ << ") *****");
    else
        ROS_INFO_STREAM("***** Incoming SPATEM indication message: counter(" << messagesCounter++ << ") *****");
    ...
    wind::cpp::SPATEM_PDU_Descriptions::SPATEM* wind = (wind::cpp::SPATEM_PDU_Descriptions::SPATEM*)buf;
    dsrc_v2_spatem_pdu_descriptions::SPATEM ros;

    wind::wind_ros::wind2ros(&ros, wind);

    if(bit0) {
        ROS_INFO_STREAM("Mirroring SPATEM message");
