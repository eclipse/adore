...
public:
    ...
    ~ReceiverNode() {
        delete udp_receiver;
        delete receiver_mcm_dmove_mcm_dmove;
    }
    ...
    void receiveMsgCallback(const void* buf, const size_t& buf_size)
    {
        ...
        if(buf_size == receiver_mcm_dmove_mcm_dmove->message_id()) {
            receiver_mcm_dmove_mcm_dmove->incoming_message(buf);
        }
        else {
    ...
    void run()
    {
        ...
        receiver_mcm_dmove_mcm_dmove = new wind::wind_ros::Receiver_mcm_dmove_mcm_dmove();
        ROS_INFO_STREAM("Registering message name(" << receiver_mcm_dmove_mcm_dmove->message_name() 
            << ") id(" << receiver_mcm_dmove_mcm_dmove->message_id() << ")");
        receiver_mcm_dmove_mcm_dmove->set_topic(topic);
        receiver_mcm_dmove_mcm_dmove->start();
...
private:
    ...
    wind::wind_ros::Receiver_mcm_dmove_mcm_dmove* receiver_mcm_dmove_mcm_dmove;
};
