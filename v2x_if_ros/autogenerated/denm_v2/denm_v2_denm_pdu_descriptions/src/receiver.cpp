...
public:
    ...
    ~ReceiverNode() {
        delete udp_receiver;
        delete receiver_denm_v2_denm_pdu_descriptions;
    }
    ...
    void receiveMsgCallback(const void* buf, const size_t& buf_size)
    {
        ...
        #if RSERIAL_ENABLED
        receiver_denm_v2_denm_pdu_descriptions->incoming_message(buf);
        #else
        if(buf_size == receiver_denm_v2_denm_pdu_descriptions->message_id()) {
            receiver_denm_v2_denm_pdu_descriptions->incoming_message(buf);
        }
        else {
    ...
    void run()
    {
        ...
        receiver_denm_v2_denm_pdu_descriptions = new wind::wind_ros::Receiver_denm_v2_denm_pdu_descriptions();
        ROS_INFO_STREAM("Registering message name(" << receiver_denm_v2_denm_pdu_descriptions->message_name() 
            << ") id(" << receiver_denm_v2_denm_pdu_descriptions->message_id() << ")");
        receiver_denm_v2_denm_pdu_descriptions->set_topic(topic);
        receiver_denm_v2_denm_pdu_descriptions->start();
...
private:
    ...
    wind::wind_ros::Receiver_denm_v2_denm_pdu_descriptions* receiver_denm_v2_denm_pdu_descriptions;
};
