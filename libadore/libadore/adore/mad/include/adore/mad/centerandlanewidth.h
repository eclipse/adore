 ...
 *
 * Contributors: 
 *   Daniel Heß - initial version for centerline computation
 *   Giovanni Lucente - addition of width computation
 ********************************************************************************/
...
namespace adore
{
	...
	{
		/**
		 *	computes the distance and the center between two point-lists of size N and M. the resulting point list has size N+M-1. N>=2 and M>=2 required.
		 *	left is of size dxN, right is of size dxM, center is of size (d+1)x(N+M-2), with d>=4, so that a matrix is ( (s0,x0,y0,z0,w0,...)^T  (s1,x1,y1,z1,w1...)^T ... )
         *  where w0, w1, ... is the width of the lane.
		 *  the center matrix must be sufficiently sized before computeLaneWidthAndCenter. 
		 *  Redundant points are removed from center, so that the actual number of distinct points is returned.
		 */
		inline int computeLaneWidthAndCenter(const adoreMatrix<double,0,0>& left,const adoreMatrix<double,0,0>& right,adoreMatrix<double,0,0>& center)
		{
			int N = left.nc();
			int M = right.nc();
			// K unused?
			...
			while(i_left<N || i_right<M)
			{
				...
				if(	std::abs(progress_RonL-progress_LonR)<min_length //same progress -> advance both
					|| (progress_LonR>=1.0 && progress_RonL>=1.0)    //both beyond next point -> advance both
					// || i_left==N && i_right==M
					)
				{
					La = Lc;
				...
				else
				{
					...
					if(i_left==N || i_right==M)
					{
						Cc = (La+Ra)*0.5;
						//@TODO set D to perpendicular distance from La or Ra to Ca->Cc 
						if(i_left<N||i_right<M) D = adore::mad::norm2<double,3>(La-Ra);
						...
						if( length_Cac > min_length )
						{
							...
							dlib::set_subm(center,d,dlib::range(i_center,i_center)) = D;
			...
			//possibly missing last point
			La = dlib::subm(left,rxyz,dlib::range(N-1,N-1));
			Ra = dlib::subm(right,rxyz,dlib::range(M-1,M-1));
			Cc = (La+Ra)*0.5;
			length_Cac = adore::mad::norm2<double,3>(Cc-Ca);
			if( length_Cac > min_length )
			{
			...
			if(i_center<2)
			{
				...
				dlib::set_subm(center,d,dlib::range(0,0)) = adore::mad::norm2<double,3>(La-Ra);
				dlib::set_subm(center,d,dlib::range(1,1)) = adore::mad::norm2<double,3>(Lb-Rb);
                i_center = 2;
			}
			return i_center;
		}

		inline int computeLaneWidthAndCenter(const LLinearPiecewiseFunctionM<double,3>& left,const LLinearPiecewiseFunctionM<double,3>& right,LLinearPiecewiseFunctionM<double,4>& center)
		{
			adoreMatrix<double, 0, 0> center_data;
			center.getData().set_size(5, std::max(left.getData().nc() , right.getData().nc())*2);
			center_data.set_size(5,std::max(left.getData().nc() , right.getData().nc())*2);
			int K = adore::mad::computeLaneWidthAndCenter(left.getData(), right.getData(), center_data);
