...
namespace adore
{
  namespace apps
  {
    ...
    class TrajectoryPlannerLC:public TrajectoryPlannerBase
    {
      ...
      adore::env::ConnectionsOnLane* connectionsOnSourceLane_;/** map controlled connections to lane*/
      adore::env::ConnectionsOnLane* connectionsOnTargetLane_;/** map controlled connections to lane*/
           ...
           coercion_detection_(&prediction_)
      {
        ...
        connectionsOnSourceLane_ = new adore::env::ConnectionsOnLane(source,&connectionSet_);
        connectionsOnTargetLane_ = new adore::env::ConnectionsOnLane(target,&connectionSet_);
        ...
        nominal_planner_ = new TNominalPlanner(
                                target_lc,
                                &ngo_,
                                connectionsOnSourceLane_,
                                connectionsOnTargetLane_,
      ...
      virtual void computeTrajectory(const adore::fun::PlanningRequest& planning_request, adore::fun::PlanningResult& planning_result) override
      {
        ...
        connectionsOnSourceLane_->update();
        connectionsOnTargetLane_->update();
        ...
        if(em_continue_active_)
        {
          ...
          if(emergency_planner2_->hasValidPlan() )
          {
            ...
            {
              planning_result.status_string = "collision detection em_continue invalid, "+ planning_result.status_string;
        ...
        {
          planning_result.status_string = "em_continue_active=false, "+ planning_result.status_string;
        }
        ...
        if(em_cancel_active_)
        {
          if(!valid_em_found)
          {
            ...
            if(emergency_planner1_->hasValidPlan() )
            {
              ...
              {
                planning_result.status_string = "collision detection em_cancel invalid, "+ planning_result.status_string;
                // planning_result.combined_maneuver.setPoints.clear();
              }            
            }
          }
        ...
        {
          planning_result.status_string = "em_cancel_active=false, "+ planning_result.status_string;
        }
