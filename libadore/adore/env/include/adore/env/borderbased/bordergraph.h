...
public:
  ...
  virtual void assign(Node* n) override
  {
    if (n->m_predecessor != 0)
    {
      ...
      {
        if (n->m_border->isNeighborOf(n->m_predecessor->m_border))
        {
    ...
    else
    {
      if (n->m_successor != 0)
      {
        ...
        {
          if (n->m_border->isNeighborOf(n->m_successor->m_border))
          {
...
public:
  ...
  public:
      ...
      : m_it(it), m_n(n), m_cost(cost), m_search_forward(search_forward)
    {
      ...
      {
        Border* neighbor = it.first->second;
        if (m_n->m_border->isContinuousPredecessorOf(neighbor) || m_n->m_border->isContinuousSuccessorOf(neighbor))
        {
          m_connectedBorders.push_back(neighbor);
        }
      }
    }
    bool hasMore()
