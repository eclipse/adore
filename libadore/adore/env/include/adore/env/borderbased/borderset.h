	...
	namespace env
	{
		namespace BorderBased
		{
			...
			public:
				 ...
				 */
				adore::mad::LLinearPiecewiseFunctionM<double,3> getCenterline(const BorderID& id)
				{
...
*/
				...
				BorderSubSet getIndexableNeighbors(Border * b)
				{
					BorderSubSet value;
					value.push_back(b);
					auto r = getRightNeighbor(b);
					while(r!=0 && r->typeIsChangeable())
					{
						value.push_back(r);
						r = getRightNeighbor(r);
					}
					auto l = getLeftNeighbor(b);
					while(l!=0 && l->typeIsChangeable())
					{
						value.push_back(l);
						l = getLeftNeighbor(l);
					}
