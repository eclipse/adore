...
namespace adore
{
	namespace mad
	{
		...
		public:
			 ...
			 */
			virtual void f(DT* xvec, CT* yvec, unsigned int count) //evaluate multi
			{
		...
		public:
			...
			virtual adoreMatrix<T, 2, 1> f(T x)override
			{
				adoreMatrix<T, 2, 1> y;
				y(0) = a->f(x);
				y(1) = b->f(x);
			...
			virtual T fi(T x, int i)override
			{
				if (i == 0)return a->f(x);
				else return b->f(x);
			}
