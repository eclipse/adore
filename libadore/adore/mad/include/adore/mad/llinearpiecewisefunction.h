	...
	namespace mad
	{
		...
		*/
		...
		public:
			...
			//binary search to find index of x, throws an exception on failure
			unsigned int findIndex(DT x, DT precision = 0.001)
			{
				if (x > limitHi() || x < limitLo())
				{
...
//					comparePointWithLine(m_data(d1, 0), m_data(d2, 0), m_data(d1, 1), m_data(d2, 1), px, py, d_tangential_i, d_normal_i);
			 ...
			 */
			double getClosestParameter(T px, T py, int d1, int d2,T& n_min)
			{
				T rel,x_min=limitLo();
			 ...
			 */
			double getClosestParameter_local(T px, T py, int d1, int d2,T x0,T* n_min=nullptr)
			{
			...
			double getClosestParameter(T px, T py, int d1, int d2)
			{
				T d_min;
		...
		private:
			...
			DT m_x1;
			DT m_dx;
			inline void idx(DT x, int dim, int & i, int & j)
			{
		...
		public:
			...
			virtual T fi(DT x, int dim) override
			{
				int i, j;
				idx(x, dim, i, j);
