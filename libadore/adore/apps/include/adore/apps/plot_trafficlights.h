  ...
  namespace apps
  {
    ...
    class PlotTrafficLights
    {
      ...
      {          
        ...
        while(tl_controller_feed_->hasNext())
        {
            ...
            if(borderSet_.size() != 0)
              figure_->plotTexture(ss_tl_icon.str(), tl_img, x_from, y_from, 1, getHeadingFromMap(x_from,y_from), 5, 2.14);
            else
              figure_->plotTexture(ss_tl_icon.str(), tl_img, x_from, y_from, 1, getHeadingOfLine(x_from,y_from,x_to,y_to), 5, 2.14);
      ...
      double getHeadingFromMap(double x, double y)
      {
          ...
          double x1 = borders_near_connection_point[0]->m_id.m_first.m_X;
          double y1 = borders_near_connection_point[0]->m_id.m_first.m_Y;
          double x2 = borders_near_connection_point[0]->m_id.m_last.m_X;
          double y2 = borders_near_connection_point[0]->m_id.m_last.m_Y;
